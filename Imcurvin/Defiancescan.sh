...
if termux-am --help >/dev/null 2>&1; then
    echo -e "\e[38;5;196m[!] SYSTEM NOTICE (PHONE/TERMUX USERS):\e[0m Do not zoom in or zoom out the terminal screen layout. Maintain default scaling to preserve CLI visual interface stability.\e[0m"
  fi
  if [ -n "$custom_proxy" ]; then
