...
reconi() {
  ...
  local c_time="[\033[34m${current_time}\033[0m]"
  
  echo -e "\n[\033[34mWARNING\033[0m] ImCurvin is designed for \033[1mauthorized security testing and educational purposes only.\033[0m"
  echo -e "Running this tool against targets without priorwritten consent is strictly illegal...\n"
  echo -e "${c_time} [i] Initiating Rapid Environmental Reconnaissance on $target_url..."

  if ! command -v whois &>/dev/null; then
    echo -e "\e[0;33m[-]\e[0m WARNING: 'whois' is not installed. Silakan instal terlebih dahulu." && exit 1
  fi

  local static_proxy="--socks5-hostname 127.0.0.1:9050"
  echo -e "${c_time} [i] Network locked to static proxy configuration [Port: 9050].\n"
  
  local tor_res=$(curl $static_proxy -m 5 -s https://torproject.org)
  if [ -z "$tor_res" ]; then
    echo -e "${c_time} \e[0;31m[!]\e[0m SOCKS5 Proxy \033[90moffline.\033[0m Aborted." >&2 && exit 1
  fi
  local tor_ip=$(echo "$tor_res" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | head -n 1)

  local target_meta=$(curl $static_proxy -m 5 -s -I "$target_url" -w "METADATA|%{remote_ip}|HTTP/%{http_version}|%{ssl_verify_result}")
  local raw_headers=$(echo "$target_meta" | grep -v "^METADATA" | tr -d '\r')
  local meta_line=$(echo "$target_meta" | grep "^METADATA")

  local target_ip=$(echo "$meta_line" | cut -d'|' -f2)
  local http_version=$(echo "$meta_line" | cut -d'|' -f3)
  local ssl_cipher=$(echo "$meta_line" | cut -d'|' -f4)
  ...
  print_status() {
    local num=$1; local label=$2; local value=$3; local default_val=$4; local is_waf=$5
    if [ -n "$value" ] && [ "$value" != "Unknown" ] && [ "$value" != "HTTP/" ]; then
      local color="\e[0;32m"; [ -n "$is_waf" ] && color="\e[0;31m"
      echo -e "${c_time} \e[0;32m[+]\e[0m ${num}. ${label} : ${color}${value}\e[0m"
    else
      echo -e "${c_time} \e[0;32m[+]\e[0m ${num}. ${label} : \033[90m${default_val}\033[0m"
    fi
  }
  echo -e "${c_time} \e[0;32m[=]\e[0m Reconnaissance Intelligence Report Consolidated:"
  print_status "1" "Outbound Gateway Proxy " "$tor_ip" "[Offline]"
  print_status "2" "Target Host Resolved IP" "$target_ip" "[Failed]"
  print_status "3" "HTTP Response Status   " "$http_status" "[No Response]"
  print_status "4" "Remote Web Server      " "$srv_info" "[Hidden/Not Detected]"
  print_status "5" "Backend Infrastructure " "$pwd_info" "[Hidden/Not Detected]"
  print_status "6" "Active Security Firewall" "$waf_info" "[No enterprise WAF detected]" "true"
  print_status "7" "HTTP Protocol Version  " "$http_version" "[Unknown]"
  print_status "8" "SSL Verification Code  " "$ssl_cipher" "[Unknown]"

  echo -e "${c_time} \e[0;32m[+]\e[0m 9. HSTS Enforcement        : $([ -n "$hsts_info" ] && echo -e "\033[32mEnabled\033[0m" || echo -e "\033[90m[Disabled / Missing]\033[0m")"
  echo -e "${c_time} \e[0;32m[+]\e[0m 10. X-Frame-Options        : $([ -n "$x_frame" ] && echo -e "\e[0;32m${x_frame}\e[0m" || echo -e "\033[90m[Missing / Clickjacking Risk]\033[0m")"
  echo -e "${c_time} \e[0;32m[+]\e[0m 11. X-Content-Type-Options : $([ -n "$x_content" ] && echo -e "\e[0;32m${x_content}\e[0m" || echo -e "\033[90m[Missing / Mime-Sniffing Risk]\033[0m")"
  echo -e "${c_time} \e[0;32m[+]\e[0m 12. X-XSS-Protection        : $([ -n "$x_xss" ] && echo -e "\e[0;32m${x_xss}\e[0m" || echo -e "\033[90m[Missing or Legacy]\033[0m")"
  echo -e "${c_time} \e[0;32m[+]\e[0m 13. Content Security Policy : $([ -n "$csp_info" ] && echo -e "\e[0;32m${csp_info}...\e[0m" || echo -e "\033[90m[Missing / High Risk]\033[0m")"
  echo -e "${c_time} \e[0;32m[+]\e[0m 14. Permissions-Policy      : $([ -n "$perm_policy" ] && echo -e "\033[32mConfigured\033[0m" || echo -e "\033[90m[Not Found]\033[0m")"
}
                helping() {
          echo -e "->>\n"
          ...
          echo -e "* \e[1mHybrid URL Parsing\e[0m: Dynamically traces \e[1mPreFlight HTTP Redirections\e[0m"
          echo -e "  to secure the absolute destination URL before passing it to localized"
          echo -e "  multi-vector attack sequences."
          echo -e ""
          echo -e "* \e[1mGoogle Dorking Reconnaissance\e[0m: Automatically discovers additional"
          echo -e "  vulnerable endpoints within the target domain to expand the attack surface."
          echo -e ""

          echo -e "\e[34mNetwork Evasion & Multi-Port TOR Routing\e[0m"
          echo -e "* \e[1mMulti-Port TOR Exporting\e[0m: Completely eliminates the typical single circuit"
          echo -e "  TOR bottleneck by exporting \e[1mmultiple active TOR ports\e[0m simultaneously"
          echo -e "  (such as \e[1m9050, 9052, and more\e[0m)."
          echo -e ""
          echo -e "* \e[1mLoad-Balanced Requests\e[0m: The Bash architecture randomly distributes parallel"
          echo -e "  \e[1mMySQL attack threads\e[0m across \e[1m6 distinct TOR circuits\e[0m to balance"
          echo -e "  outbound network traffic."
          echo -e ""
          echo -e "* \e[1mContinuous Identity Mutation\e[0m: Pushes evasion to its \"limits\" through"
          echo -e "  \e[1mdynamic multi-IP TOR circuit rotations\e[0m per request combined with"
          echo -e "  \e[1mautomated UserAgent mutations\e[0m on every concurrent thread."
          echo -e ""
          echo -e "* \e[1mRandomized Delay Insertion (Jitter)\e[0m: Introduces unpredictable,"
          echo -e "  \e[1mnon-linear time intervals\e[0m between concurrent requests to destroy the"
          echo -e "  traffic-pattern baselines of behavioral AI filters."
          echo -e ""

          echo -e "\e[34mAdvanced WAF Bypass & Obfuscation\e[0m"
          echo -e "The engine deploys a \e[1msynchronized dual-vector parallel attack\e[0m that"
          echo -e "simultaneously probes MySQL time-based anomalies using multi-layered payload"
          echo -e "obfuscation and intelligent header injection, featuring:"
          echo -e ""
          echo -e "* \e[1mStacked Queries Injection\e[0m: Utilizes the stacked query technique (\e[1m;\e[0m)"
          echo -e "  to terminate the application's original database query and force the"
          echo -e "  independent execution of injected dynamic SQL statements."
          echo -e ""
          echo -e "* \e[1mHTTP Parameter Pollution (HPP) Splitting\e[0m: Injects duplicated query"
          echo -e "  parameters with identical names (\e[1mparam_name=999&param_name=payload\e[0m) to"
          echo -e "  exploit parsing discrepancies between the front-end WAF and the back-end"
          echo -e "  application server, effectively masking malicious database payloads behind"
          echo -e "  benign values."
          echo -e ""
          echo -e "* \e[1mPayload Pen-Testing Masking\e[0m: Conceals detection signatures through"
          echo -e "  \e[1mrandomized case conversion, space2comment encoding, XOR encryption\e[0m,"
          echo -e "  and \e[1mBase64 encoding matrices\e[0m."
          echo -e ""
          echo -e "* \e[1mIntelligent Header Injection\e[0m: Injects spoofed IP headers,"
          echo -e "  \e[1mCloudflare bypass chains\e[0m, and cache-control directives tailored"
          echo -e "  specifically to complex URL parameter structures."
          echo -e ""
          echo -e "* \e[1mProtocol Exploitation\e[0m: Executes synchronized \e[1mJA3/JA4 TLS Fingerprint\e[0m"
          echo -e "  \e[1mSpoofing, HTTP/2 Rapid Reset Protocol Exploitation\e[0m via a customizable"
          echo -e "  high-concurrency window (defaulted to \e[1m50 concurrent streams\e[0m),"
          echo -e "  \e[1mHTTP Chunked Transfer Encoding Mismatch\e[0m, and \e[1mAutomated Control Loop\e[0m"
          echo -e "  \e[1mShadowban Evasion\e[0m."
          echo -e ""

          echo -e "\e[34mAutomated IP Re-Birth via TOR NEWNYM\e[0m"
          echo -e "To ensure uninterrupted execution against active threat mitigation systems,"
          echo -e "imcurvin integrates an automated defensive evasion loop:"
          echo -e ""
          echo -e "* \e[1mBlock Detection\e[0m: If the target infrastructure responds with a block"
          echo -e "  status (such as \e[1mHTTP 403 Forbidden\e[0m or \e[1m429 Too Many Requests\e[0m),"
          echo -e "  the engine dynamically triggers a \e[1m\"SIGNAL NEWNYM\"\e[0m instruction across"
          echo -e "  the active port array."
          echo -e ""
          echo -e "* \e[1mInstant Rotation\e[0m: Instantiates an immediate circuit teardown and rebuild,"
          echo -e "  rotating the outbound exit node mapping \e[1mwithin milliseconds\e[0m"
          echo -e "  without interrupting the primary multi-threaded attack vector."
          echo -e ""

          echo -e "\e[34mPost-Scan Validation Engine\e[0m"
          echo -e "* \e[1mLatency Isolation\e[0m: Every time-based anomaly generated during the"
          echo -e "  attack is filtered by a specialized post-scan \e[1mPython validation engine\e[0m"
          echo -e "  designed to isolate baseline network latency from genuine database"
          echo -e "  thread delays."
          echo -e ""
          echo -e "* \e[1mEnvironment Safeguard\e[0m: The framework detects \e[1mnon-MySQL environments\e[0m"
          echo -e "  and aborts execution to prevent \"resource wastage.\""
          echo -e ""
          exit 0
        }  
