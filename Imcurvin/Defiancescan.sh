...
if termux-am --help >/dev/null 2>&1; then
    echo -e "\e[38;5;196m[i] SYSTEM NOTICE (TERMUX USERS):\e[0m Are you using Termux? If you zoom in excessively and experience layout tearing, please zoom out to restore interface alignment"
  fi
  if [ -n "$custom_proxy" ]; then
